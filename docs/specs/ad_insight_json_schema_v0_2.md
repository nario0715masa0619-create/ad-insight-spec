...
```json
{
  "input_metadata": { ... },      // どのモードで入力されたか
  "asset_meta": { ... },          // 素材メタデータ（file-first）
  "creative_core": { ... },       // 素材コンテンツ分析
  "landing_page": { ... },        // LP分析（optional）
  "performance": { ... },         // KPI（optional）
  "diagnostics": {
    "qualitative": { ... },       // KPIなしでも可（LLM分析）
    "quantitative": { ... }       // KPI必須（performance-based）
  },
  "views": { ... },               // UI表示用（生成版）
  "_metadata": { ... }
}
