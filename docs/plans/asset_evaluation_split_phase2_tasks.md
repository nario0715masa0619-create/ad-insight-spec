...
**ステータス**: 🟢 Phase 1（カラム＋v0スキーマ）実装済み・ローカル検証済み ／ Phase 2第一段階（adapter scaffold）実装済み（PR #61）／ downcast第一バッチ〜バッチ2（`asset_meta`/`input_metadata`/`creative_core`の3ブロック分）実装済み（PR #65〜#68） ／ `diagnostics`/`performance`/`landing_page`/`views`/`_metadata`のdowncastと`resolve_spec_data`への配線は引き続き未実装
**実装状況**:
...
9. **[downcast本体]** `asset_data + evaluation_data → spec_data`の実際の変換コード — 🟡 3/8ブロック完了（2026-07-09、PR #65, #68）。`asset_meta`/`input_metadata`/`creative_core`は実装・テスト済み。残り: `diagnostics`（video_cutsのcut_id補完含む）/`performance`/`landing_page`/`views`/`_metadata`（`json_schema_version`復元含む）。全ブロック完了後に`resolve_spec_data`へ一括配線する方針（adapterモジュールdocstring参照）
...
- `backend/app/services/asset_evaluation_adapter.py` — `resolve_spec_data`＋downcast部品3つ（`_downcast_asset_meta`/`_downcast_input_metadata`/`_downcast_creative_core`、いずれも未配線）
...
最終更新: 2026-07-09
ステータス: Phase 1（カラム＋v0スキーマ）実装・ローカル検証済み、本番適用は未実施 ／ Phase 2 adapter scaffold・specs.py配線 実装済み ／ downcast: `asset_meta`/`input_metadata`/`creative_core`の3ブロック実装・テスト済み（PR #65, #68） ／ 残り5ブロック（`diagnostics`/`performance`/`landing_page`/`views`/`_metadata`）とその後の`resolve_spec_data`配線は未着手
