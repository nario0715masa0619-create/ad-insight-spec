...
**ステータス**: 🟢 Phase 1（カラム＋v0スキーマ）実装済み・ローカル検証済み ／ Phase 2第一段階（adapter scaffold）実装済み（PR #61）／ downcastバッチ1〜3（`asset_meta`/`input_metadata`/`creative_core`/`diagnostics`の4ブロック分）実装済み（PR #65〜#68、#69） ／ 残り`performance`/`landing_page`/`views`/`_metadata`は分類のみ完了・実装は未着手 ／ `resolve_spec_data`への配線は引き続き未実装
**実装状況**:
...
- **Phase 2 downcastバッチ3（2026-07-09、PR #69）**: 優先順位（diagnostics→performance→landing_page→views→_metadata）に沿って残り5ブロックを実コード調査で分類し（下記「🧩 残り5ブロックのlossless/近似/埋められない分類」参照）、最優先の`diagnostics`のみ実装。
  ...
  - `performance`/`landing_page`は型が完全一致のためdowncastは恒等写像になる見込み（未実装）、`views`は現行legacyでもほぼ全て固定値と判明（未実装）、`_metadata`は`json_schema_version`の復元方法のみ未決定（未実装）
...
### 2. `performance`（分類のみ、未実装）

`EvaluationJsonV0.performance: Optional[Performance]`は既存`Performance`型をそのまま再利用
しているため、**全フィールドがlossless**（型が完全一致、変換ロジック不要、実質的に
`spec_data.performance = evaluation_data.performance`という恒等写像）。downcast関数を作るとしても
中身は「そのまま返す」だけになる。

### 3. `landing_page`（分類のみ、未実装）
...
`views`はasset_data/evaluation_dataの実データを一切必要とせず、downcast関数は引数を無視して
上記の固定値dictを返すだけで legacy 挙動と完全に一致する。ただし「本当にこの固定値のままで
良いのか（将来Viewsを実データ化する予定はあるか）」はこのバッチのスコープ外の論点として残す。

### 5. `_metadata`（分類のみ、未実装、オープン課題3の続き）
...
9. **[downcast本体]** `asset_data + evaluation_data → spec_data`の実際の変換コード — 🟡 4/8ブロック完了（2026-07-09、PR #65, #68, #69）。`asset_meta`/`input_metadata`/`creative_core`/`diagnostics`は実装・テスト済み。残り: `performance`/`landing_page`（型完全一致のため恒等写像の見込み）/`views`（現行legacyでもほぼ固定値と判明）/`_metadata`（`json_schema_version`復元のみ未決定）。全ブロック完了後に`resolve_spec_data`へ一括配線する方針（adapterモジュールdocstring参照）
...
- `backend/app/services/asset_evaluation_adapter.py` — `resolve_spec_data`＋downcast部品4つ（`_downcast_asset_meta`/`_downcast_input_metadata`/`_downcast_creative_core`/`_downcast_diagnostics`、いずれも未配線）
...
最終更新: 2026-07-09
ステータス: Phase 1（カラム＋v0スキーマ）実装・ローカル検証済み、本番適用は未実施 ／ Phase 2 adapter scaffold・specs.py配線 実装済み ／ downcast: `asset_meta`/`input_metadata`/`creative_core`/`diagnostics`の4ブロック実装・テスト済み（PR #65, #68, #69） ／ 残り4ブロック（`performance`/`landing_page`/`views`/`_metadata`）は分類済み・実装は未着手、その後の`resolve_spec_data`配線も未着手
