...
6. 実ブラウザでの最終確認（完了条件）
   - `streamlit.testing.v1.AppTest` によるスクリプトレベルの再現・解消確認は必須だが、それだけでは完了扱いにしない。
   - 実ブラウザ（Chrome拡張等）が使える状態になったタイミングで、影響を受けたタブ（Analyze / List / Detail / Delete など）を実際に開いて目視確認し、Detail / Deleteの主要ボタンを数件クリックして期待どおりの挙動になることを確認する。
   - ブラウザ接続が使えない状況で修正を報告する場合は、「コード修正・AppTest検証は完了、実ブラウザ確認は未実施（接続可能時に追加実施予定）」の形で未確認事項として明示すること。実ブラウザ確認が完了して初めて、一覧UI修正はフル完了として扱う。
